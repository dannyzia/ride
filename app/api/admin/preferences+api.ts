...
export async function POST(req: Request) {
  ...
  try {
    ...
    logger.info('[admin/preferences] created', { id: pref.id, name: pref.name });
    return Response.json({ preference_id: pref.id }, { status: 201 });
  } catch (err: any) {
    if (err.code === '23505') {
      return Response.json({ error: 'preference_name_exists' }, { status: 409 });
    }
...
export async function PATCH(req: Request) {
  ...
  if (data.display_label_en !== undefined) updates.display_label_en = data.display_label_en;
  if (data.display_label_bn !== undefined) updates.display_label_bn = data.display_label_bn;
  if (data.icon !== undefined) updates.icon = data.icon;
  if (data.charge_bdt !== undefined) updates.charge_bdt = data.charge_bdt;
  if (data.affects_matching !== undefined) updates.affects_matching = data.affects_matching;
  ...
  const [pref] = await db.update(preferences)
    .set(updates)
    .where(eq(preferences.id, prefId))
    .returning();

  if (!pref) return Response.json({ error: 'preference_not_found' }, { status: 404 });
