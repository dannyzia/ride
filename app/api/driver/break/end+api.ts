...
export async function POST(request: Request) {
  try {
    ...
    await db.update(drivers)
      .set({ on_break: false, break_started_at: null, updated_at: now })
      .where(eq(drivers.id, driver.id));

    return Response.json({ on_break: false, break_started_at: null }, { status: 200 });

  } catch (err: any) {
    if (err.status === 401) return Response.json({ error: "unauthorized" }, { status: 401 });
    logger.error("[driver/break/end] error", err);
