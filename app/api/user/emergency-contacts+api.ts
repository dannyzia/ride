...
export async function POST(request: Request) {
  try {
    ...
    // Max-5 gate: count existing contacts before insert
    const [cnt] = await db
      .select({ value: count() })
      .from(userEmergencyContacts)
      .where(eq(userEmergencyContacts.user_id, dbUser.id));

    if (cnt && cnt.value >= MAX_CONTACTS) {
      return Response.json(
        {
          error: "max_contacts_reached",
          message: `Maximum of ${MAX_CONTACTS} emergency contacts allowed`,
        },
        { status: 422 },
      );
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!contactId)
      return Response.json(
        { error: "invalid_uuid", message: "Contact id is required" },
        { status: 400 },
      );
    if (!z.string().uuid().safeParse(contactId).success)
      return Response.json(
        { error: "invalid_uuid", message: "Invalid UUID format" },
        { status: 400 },
      );

    await db
      .delete(userEmergencyContacts)
      .where(
        and(
