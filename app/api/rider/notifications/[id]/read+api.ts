// PATCH /api/rider/notifications/[id]/read — mark one notification read.
// Expo API routes pass dynamic params directly (NOT wrapped in { params }).
// Bodyless PATCH (no request body to validate).
...
export async function PATCH(request: Request, { id }: { id: string }) {
  try {
    ...
    logger.error('[rider/notifications/read] error', err);
