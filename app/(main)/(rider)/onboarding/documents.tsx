...
const REQUIRED_DOCUMENTS = [
  ...
  },
  {
    ...
    description: "Vehicle registration paper (if applicable)",
  },
];
    ...
    <SafeAreaView className="flex-1 bg-goBgColor">
      ...
      >
        ...
        <View className="flex-row items-center justify-between mt-4 mb-6">
          ...
          <TouchableOpacity
            onPress={() => setStep(step === "upload" ? "consent" : "upload")}
          >
            <Text className="text-accentColor text-sm">
              {step === "upload" ? "Next" : "Back"}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View className="flex-row justify-center mb-6">
          <View
            className={`w-3 h-3 rounded-full mx-1 ${step === "upload" ? "bg-accentColor" : "bg-borderColor"}`}
          />
          <View
            className={`w-3 h-3 rounded-full mx-1 ${step === "consent" ? "bg-accentColor" : "bg-borderColor"}`}
          />
        </View>
        ...
        {step === "upload" && (
          <>
            <Text className="text-secondaryTextColor text-sm mb-5">
              Upload clear photos of the following documents. Accepted formats:
              JPG, PNG.
            </Text>
            ...
            {REQUIRED_DOCUMENTS.map((doc) => (
              <View
                key={doc.key}
                className="bg-cardBgColor rounded-xl p-4 mb-3"
              >
                <View className="flex-row items-center justify-between">
                  <View className="flex-1 mr-3">
                    ...
                    <Text className="text-secondaryTextColor text-xs mt-1">
                      {doc.description}
                    </Text>
                  </View>
                  {documents[doc.key] ? (
                    ...
                    >
                      <View className="flex-row items-center">
                        ...
                        />
                        <Text className="text-success-500 text-xs ml-1">
                          Done
                        </Text>
                      </View>
                    </TouchableOpacity>
                    ...
                    >
                      {uploading === doc.key ? (
                        ...
                        <Text className="text-accentColor text-sm font-medium">
                          Upload
                        </Text>
                      )}
                    </TouchableOpacity>
        ...
        {step === "consent" && (
          <>
            ...
            {CONSENT_ITEMS.map((item, index) => (
              ...
              >
                <View
                  className={`w-5 h-5 rounded border-2 mr-3 mt-0.5 items-center justify-center ${consentChecked[index] ? "bg-accentColor border-accentColor" : "border-borderColor"}`}
                >
                ...
                <Text className="text-primaryTextColor flex-1 text-sm leading-5">
                  {item}
                </Text>
              </TouchableOpacity>
            ))}
            ...
            >
              {submitting ? (
                ...
                <Text
                  className={`text-base font-bold ${allConsentGiven ? "text-white" : "text-secondaryTextColor"}`}
                >
                  Submit Documents
                </Text>
              )}
