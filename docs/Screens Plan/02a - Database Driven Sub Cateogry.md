...
|---|------|--------|-----------------|
| 1 | **`VEHICLE_CATEGORIES` constant** — centralize the 4 parent categories (`key`, `display_en`, `display_bn`, icon, sort order) in `lib/vehicleTypes.ts`; have Services Hub + Home consume it instead of each holding a separate piece. | Small | ✅ Done (verified 2026-08-14) |
| 2 | **i18n** — render category/type labels via `display_en` / `display_bn` (Bengali). | Small | ✅ Done (verified 2026-08-14) |
