...
## v2.1 — Implementation Report (Code Complete; Validation Pending) — 2026-08-15

> **Status:** Waves 1–3 IMPLEMENTED. This document now serves as the **spec of record** for what was built, plus the verification evidence. **NOT merged** until §8's pending validation commands pass.
>
...
```
- Writes `vehicles`: `manufacturer=brand`, `model`, `manufacturing_year=registration_year` (**D-1**), `registration_date=today` (documented placeholder; admin-correctable), real plate parsing kept from existing logic where sane.
...
**Route contract (exact):** `POST {UTILS_SERVER_URL}/internal/ride/cancelled` (utils-server reuses its internal HTTP listener; same base-URL resolution and `Authorization: Bearer ${WEBSOCKET_INTERNAL_SECRET}` guard as `/internal/ride/completed`). Payload `{ ride_id: uuid, driver_id: uuid }`. Server sends `{ type: "ride:cancelled", ride_id }` to the driver's socket.
...
- **Delete** legacy `POST /api/driver/verify-driver` usage from this screen. ✅ done — client usage removed AND the endpoint file itself is now a **410 Gone stub** (`app/api/driver/verify-driver+api.ts`, same pattern as the inert `payment/bkash` stubs), since zero callers remained.
