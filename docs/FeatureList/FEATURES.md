...
> v25 — 2026-09-25: R2 UPLOAD DEVICE-VERIFIED (PASS) + TEST-GUARD HARDENING + DEVICE-DAY DB BLOCKER RESOLVED. R2 presigned-upload pipeline verified end-to-end on device (profile photo through native 1:1 crop + vehicle document, no crop): both PUTs 2xx with client-emitted `Content-Type`/`Cache-Control` byte-identical to the signed values; R2 HEAD responses confirm stored headers; `PATCH /api/driver/me` + document-key upload confirmed; test baseline restored after (DB null, R2 prefixes empty, public URLs 404) — report: `DEVICE-DAY-QA-2026-09-25-R2-UPLOAD.md` (Device Day item 7 → PASS). i18n interpolation guard + locale fixes (ISSUE-69, `b9b9b50`); router-tree default-export guard promoted from the ISSUE-70 one-off sweep (`fe46993`, `lib/__tests__/router-route-defaults.test.ts`). The Supabase DB outage that produced the 2026-09-23 NOT READY verdict (GO ONLINE → `CONNECT_TIMEOUT` pooler) was external and is resolved — device login/API/auth working again 2026-09-25; no code change was required. Remaining device-day items: GO ONLINE re-confirm (expected pass), item 2 fare-regression full path, C1 marketplace verticals, i18n full-Bangla flow, Bengali numerals, size-round splash.
> v24 — 2026-09-24: STORAGE MIGRATION (CLOUDFLARE R2) + I18N BENGALI NUMERALS & FLOATING TOGGLE + DRIVER DISPATCH HARDENING + DEVICE DAY QA REPORT (HEAD `56ce3ace`). Migrated file storage from Supabase to Cloudflare R2 (`285c45a0`–`56ce3ace`); added free-floating circular language toggle (`6167f188`, ISSUE-65) and Bengali numerals P1–P3 (`ca68e545`); unified driver online-state ownership (`919cd2b8`, `19ee073b`), presence grace contract (`054fc581`, `e8bee5d2`), bounded location reporting across tabs (`8a3c9294`, `88949140`); bounded DB transaction idle time (`ff407482`, ISSUE-62); added `GET /api/delivery/requests/[id]` (`97596e3f`). Device Day QA (2026-09-23) conducted on Pixel 6a + Redmi — verdict NOT READY (critical blocker: driver GO ONLINE failing on Supabase pooler connection timeout `CONNECT_TIMEOUT aws-1-ap-northeast-1.pooler.supabase.com:6543`).
...
| 60 | App language (EN/BN) + Floating toggle + Bengali numerals | YES (`i18n/i18n.ts` — react-i18next + AsyncStorage) | YES | YES — i18next with REAL Bangla strings (`i18n/locales/{en,bn}/common.json`); **R2.4 sweep wired 60/63 rider screens + driver screens**; **free-floating circular toggle (EN ↔ বাংলা)** on rider screen (`6167f188`, ISSUE-65); **Bengali numerals P1–P3** (`formatBDT`, `useTaka` hook, `ca68e545`) supporting English digits `৳1,234` vs Bengali digits `৳১২,৩৪,৫৬৭`; rider selector at `settings/app-language`, driver selector at `settings/language` |
| **Public** | | | | |
...
| 6 | Personal profile (name, photo, city) | YES | YES | YES — photo uploaded to Cloudflare R2 presigned storage (`285c45a0`, `7e781d68`, `792c7209`, `56ce3ace`); **device-verified 2026-09-25** incl. native crop path (`DEVICE-DAY-QA-2026-09-25-R2-UPLOAD.md`) |
| **Home / Status** | | | | |
| 7 | DriverHome (map + WS + heartbeat) | YES | YES | YES — tab: Home; identity-checked WS adoption + module-level message-handler indirection (H-1); reconnect re-fetches session; location reporting active across all tabs while online (`8a3c9294`), bounded fix + last-known fallback (`88949140`) |
...
| 10 | Go Online / Go Offline | YES | YES | YES — single owner for online flag & session (`919cd2b8`, `19ee073b`); lost socket holds online state via presence grace contract (`054fc581`, `e8bee5d2`); self-diagnosing heartbeat silence (`2dc1d53a`) |
...
| 15 | Delivery request creation + bid/accept/leg lifecycle | YES | YES | YES — `requireCourier` gate; partial unique on `delivery_bids` (F3); `GET /api/delivery/requests/[id]` detail route (`97596e3f`) |
...
- **Storage (Cloudflare R2)**: File uploads migrated from Supabase Storage to Cloudflare R2 (`285c45a0`, `7e781d68`, `792c7209`, `56ce3ace`); shared presigned upload contract, strict validation gates, and Node global crypto key suffixing.
- **Database Watchdog & Connection Safety**: Pool health watchdog evaluates delivered work rather than queued probe queries (`508aedd8`, `776b0fe4`); transaction safety bounds idle-in-transaction timeouts across all transactions (`ff407482`, ISSUE-62).
...
| Marketplace deployment gates | **High (pre-launch)** | Code COMPLETE + audit-fixed + regression-tested (`2db2964`, 148 suites / 1903 green). Flags LIVE (`marketplace_*_enabled` + `auto_redispatch_enabled` = true). **Device Day QA 2026-09-23: NOT READY — root cause was the external Supabase DB outage (`CONNECT_TIMEOUT` pooler → `auth:error: server_unavailable`), resolved by 2026-09-25 (device login/API/auth working; no code change required)**. R2 upload verified PASS on device 2026-09-25 (`DEVICE-DAY-QA-2026-09-25-R2-UPLOAD.md`). GO ONLINE needs re-confirm only (expected pass post-DB-restore); C1 verticals + fare-regression full path still unexecuted on device; A8 load test remains post-flip watch |
...
| Maestro device E2E pass (booking path, SOS, payments, marketplace verticals) | Recommended | Device run 2026-09-23 blocked on driver online transition; **DB blocker resolved 2026-09-25 — ready to re-run**. Pre-run checklist: `node scripts/dev-env-sync.js` BEFORE `npx expo start` (`TEST-SETUP.md`), utils-server up (port 3001), fresh dev-client bundle if env changed. Re-run: GO ONLINE re-confirm, booking + fare-regression full path (item 2), SOS, payment, marketplace C1 verticals (items 3–6, 8) (`DEVICE-DAY-QA-2026-09-23.md`, `DEVICE-DAY-QA-2026-09-25-R2-UPLOAD.md`) |
