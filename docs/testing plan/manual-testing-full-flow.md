...
- **Login is password-based** (existing accounts log in with phone + password; OTP is registration-only). **In this dev environment `DEV_OTP_BYPASS=true` is enabled** — any phone's OTP is the fixed code **`123456`** (configurable via `DEV_OTP`), which covers the registration/OTP screens. No SMS needed. If a session is already active (app not killed), login can be skipped.
