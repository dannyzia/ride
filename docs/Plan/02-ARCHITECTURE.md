...
- `app/(admin)/` — Admin web panel route group: approval queue, KYC review, package management, zone config, city-boundary config, pricing, heatmap.
...
- `lib/cityBoundary.ts` — Loads active `city_boundaries`, runs point-in-polygon checks, and resolves `{origin_city, is_intercity}` for pickup/dropoff. City polygons are DB data. **Cache strategy:** In-process memory cache with a 60-second TTL (default `CITY_BOUNDARY_CACHE_TTL_MS=60000`). On cache miss, query all active rows from `city_boundaries` and store them in a module-level Map. Cache is invalidated on every `PATCH /api/admin/city-boundaries/:id` and `DELETE /api/admin/city-boundaries/:id` by calling `cityBoundaryCache.clear()` in the route handler. This is MVP simplicity; post-MVP may move to Redis or a shared cache.
- `lib/routeSplit.ts` — Primary path: calls the Barikoi v2 route API (`https://barikoi.xyz/v2/api/route/{lng},{lat};{lng},{lat}?api_key=...&geometries=polyline`) with origin and destination coordinates. (The previously documented `v1/api/directions/v1/driving` path returns HTTP 404 — dead; see lib/routeSplit.ts header note.) The API key is read from `BARIKOI_API_KEY` (see 11-ENV-VARS.md). The returned Google-encoded polyline is decoded into an array of `[lng, lat]` coordinates using a polyline decoder utility (e.g., `@mapbox/polyline` or a 20-line inline decoder), then converted to GeoJSON LineString for Turf.js. Using `@turf/turf`, the line is split against the origin city polygon; segment lengths inside and outside the polygon are computed with `turf.length()` and returned as `{inside_km, outside_km}` rounded to 3 decimals. **Fallback:** If Barikoi returns non-200 or times out (>5s), compute straight-line Haversine distance and apply an urban factor of 1.3 for the inside-origin-city segment and 1.0 for the outside segment. The fallback assumes the entire distance is inside if dropoff is also inside the origin city; otherwise it allocates proportionally based on the straight-line ratio of pickup-to-city-centroid vs pickup-to-dropoff distances.
...
Rider app
  ...
  → lib/zone.ts: isInsideZone(pickup)  [422 if outside operational zone]
      → Active `zones.polygon` is the simplified Bangladesh mainland border from `ifahimreza/bangladesh-geojson`; this checks whether Ride operates at the pickup, not whether a ride is intercity.
  → lib/cityBoundary.ts: detect origin city
      → Point-in-polygon pickup against all active `city_boundaries`.
      → If pickup is inside a city: set `origin_city = city.name`.
      → If pickup is not inside any city: set `origin_city = null`, `is_intercity = false`, `outside_km = 0`.
      → If pickup has an origin city: point-in-polygon dropoff against that same city polygon. Dropoff outside origin city means `is_intercity = true`.
  → lib/routeSplit.ts: resolve route distance split
      → Non-intercity or rural-origin ride: `inside_km = route_distance_km`, `outside_km = 0`.
      → Intercity ride: call Barikoi Route API with `geometries=polyline`, split the returned road geometry with Turf.js against the origin city polygon, and compute `inside_km`/`outside_km` rounded to 3 decimals.
      → Fallback if Barikoi is unavailable: Haversine estimate with a 1.3 urban factor for inside-origin-city distance and 1.0 for outside distance.
  → lib/fareCalc.ts: computeBreakdown()
      → **Distance source:** `routeSplit` output for fare distance. Barikoi route geometry is preferred for intercity splitting; fallback is straight-line Haversine as described above.
      → Applies formula: `inside_charge = round(per_km_bdt × inside_km)`; `outside_charge = round(intercity_per_km_bdt × outside_km)`; `distance_charge = inside_charge + outside_charge`; at request time `ride_time_min = 0` (timer not started); `computed_total = base_fare_bdt + distance_charge`; `floor_fare = base_fare_bdt + round(per_km_bdt × floor_length_km) + (floor_min × per_min_bdt)`; `final_fare = max(computed_total, floor_fare)`
      ...
      → Returns {base_fare_bdt, distance_charge_bdt, time_charge_bdt, total_bdt, floor_fare_bdt, distance_km, origin_city, is_intercity, inside_km, outside_km, inside_charge_bdt, outside_charge_bdt}
      ...
      → **Note:** Fare distance uses Barikoi route geometry when route splitting is needed. ETA for `rides.eta_minutes` can still use Google Maps Distance Matrix API (faster, no route details needed) or the configured ETA provider.
