...
>
> **RLS Note (updated 2026-09-25, T6 → ISSUE-80/81 end state):** Row Level Security is ENABLED + FORCED on all public tables (`scripts/rls-enable-force.sql`), with **zero policies and zero anon/authenticated table grants** (`scripts/rls-revoke-broad-grants.sql` Phases A+B; `scripts/rls-drop-users-self-read.sql` dropped the former `users_self_read` whitelist + its paired grant) — full default-deny for client keypaths; anon/authenticated writes probe as 42501. The app's data path is unaffected — Drizzle connects as the `postgres` role (BYPASSRLS); middleware auth (`lib/auth.ts`) still handles API-layer authorization, and the admin panel resolves roles server-side via `/api/auth/verify-token` (`lib/adminRoleClient.ts`, guarded by `tests/components/admin-no-client-table-reads.test.ts`). TD-31 alignment: any future anon/authenticated table access needs an explicit GRANT + policy. History: RLS was first enabled during the 2026-08-14 Security Advisor campaign (`docs/errors/2026-08-14-supabase-security-advisor-fixes.md`).
