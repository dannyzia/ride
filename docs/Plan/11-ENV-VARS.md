...
| `BARIKOI_API_KEY` | yes | — | Barikoi Maps API key. Used server-side for geocoding/routing (`lib/barikoi.ts`) and exposed to client via `app.config.js` → `extra.EXPO_PUBLIC_BARIKOI_API_KEY`. | Barikoi developer portal |
