...
| `BARIKOI_API_KEY` | yes | — | Barikoi Maps API key. Used server-side for geocoding/routing (`lib/barikoi.ts` and `lib/routeSplit.ts`) and exposed to client via `app.config.js` → `extra.EXPO_PUBLIC_BARIKOI_API_KEY`. **Route API usage:** `lib/routeSplit.ts` calls the v2 route endpoint `https://barikoi.xyz/v2/api/route/{lng},{lat};{lng},{lat}?api_key=...&geometries=polyline` with this key (the older `v1/api/directions/v1/driving` path is dead — returns HTTP 404) to get road geometry for intercity route splitting. The encoded polyline response is decoded and processed with Turf.js. If this key is missing or the API is unreachable, `lib/routeSplit.ts` falls back to Haversine estimation with urban factors. | Barikoi developer portal |
