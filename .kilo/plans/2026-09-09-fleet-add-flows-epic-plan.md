...
- **Phase A (the epic’s core, ~1.5 sessions):** §4.1, §4.2 + `lib/fleetLimits.ts` tx-handle refactor + tests for all four AC-4 cells per endpoint. No transfer logic, no remove path (R4). **[EXECUTED 2026-09-09]** — Zia pulled the §4.4 error-first DELETE forward into Phase A; see ruling record for the delta list.
...
2026-09-09 — Phase A executed (ISSUE-35): POST /api/fleet/vehicles, POST /api/fleet/assignments, DELETE /api/fleet/vehicles/[id] (error-first per R4, pulled forward from Phase B by Zia), lib tx-handle refactor + fleetLimitLockKey/takeFleetLimitLock. Schema-reality deltas applied during implementation: (1) registration_number is GLOBALLY unique (vehicles_reg_number_idx) — explicit dup probe + 23505 fallback in the route; D5 per-fleet framing corrected; (2) staff add requires the full vehicles NOT NULL surface (manufacturer/model/manufacturing_year/passenger_seats required — §4.1 optional-fields framing corrected); (3) FOLLOWUP-A/B conventions mirrored (registration_area default DHAKA_METRO, vehicle_class_letter default KA); (4) D4 enum gate confirmed from fleetStatusEnum (ACTIVE-only). Tests: tests/api/fleet/fleet-add-flows.test.ts (29 tests) — AC-4 matrix, role/status gates, advisory-lock call-order, error-first DELETE paths (vehicle_in_use / vehicle_has_history / 404s / invalid_uuid), zero-writes-on-failure, real fleetAssignment lib exercised in-tx. Gates: 1972/1972 full suite, tsc 0/0 (root + utils-server), lint 0 errors.
REPAIR DISCLOSURE (2026-09-09, same day): the Phase A implementation initially overwrote app/api/fleet/assignments+api.ts — §2 above was WRONG that no route exposed assignVehicleToDriver: the route shipped in 54c8466 (universal fleet model phase) with POST (OWNER/MANAGER) + PATCH unassign via unassignVehicle, which IS the only unassign path. Repaired by merging both: PATCH restored verbatim; POST deltas vs the pre-existing route (disclosed): roles widened to +DISPATCHER per §4.2, assigned_by now carries the USERS id (schema FK — the old code passed the fleet_members row id, not a users.id), FleetAssignmentError maps to 404 (was 422). §2 table row corrected by this note; tests added for the restored PATCH path (suite 33 tests; full suite 1976/1976, tsc 0/0, lint 0 errors). Commit: df3c669 (amended; f69c532 pre-amend discarded as unpushed).

2026-09-09 — Phase B executed (ISSUE-35): POST /api/fleet/drivers (§4.3 attach-or-transfer per R1) — commit 4c498e0. Scope delta vs plan: §4.4 error-first vehicle removal was already pulled forward into Phase A by Zia (ruling R4 record), so Phase B = §4.3 only. Schema-reality deltas applied: (1) drivers.vehicle_type is NOT NULL with no default — the A1 provision INSERT supplies the register+api.ts precedent default "bike_basic" (§4.3's "vehicle fields NULL" holds for vehicle_id / min_per_km_bdt, which stay unset); (2) D1's "own no vehicles" transfer condition is subsumed by the B2 active-assignment guard — unassigned vehicles have cleared driver pointers (lib/fleetAssignment invariant), so an active assignment is the only ownership surface that can strand a cross-fleet pointer; no additional vehicle-probe needed. Implementation: POST handler on app/api/fleet/drivers+api.ts (GET unchanged) — tx → takeFleetLimitLock → D4 ACTIVE gate → checkDriverLimit on tx handle → user probe (404 user_not_found) → A1/A2/B1/B2 branch dispatch; B2 guard = active fleet_vehicle_assignments row (isNull(unassigned_at), fva_driver_active_idx-backed) OR is_online; transfer is a single drivers.fleet_id UPDATE, old fleet untouched (Phase C defers demotion ceremony). Tests: 13 new endpoint tests in tests/api/fleet/fleet-add-flows.test.ts (46 total) — full branch matrix, at-limit 403 with current/limit, lock/check call-order, tx-handle check, SUSPENDED gate, user 404, role matrix, invalid param/body, zero-writes on every failure path. Gates: fleet suites 90/90, full suite 1995/1995 (156 suites), tsc 0/0 (root + utils-server), lint 0 errors (285-warning baseline unchanged — stash-verified), eq(col,null) scan clean, vacuous gate clean at commit.
