...
// Fleet assignment writes are out of scope for classification tests — mock
// the sole write path (lib/fleetAssignment.ts) so the route's fleet wiring
// doesn't need fleet tables in the DB mock.
jest.mock('@/lib/fleetAssignment', () => ({
  assignVehicleToDriver: jest.fn(() => Promise.resolve({ assignment_id: 'assign-1' })),
  unassignVehicle: jest.fn(() => Promise.resolve()),
  FleetAssignmentError: class FleetAssignmentError extends Error {
    constructor(code = 'error', message = 'Fleet assignment error') {
      super(message);
...
function queueNewVehicleSelects(model: Record<string, unknown> | null = null) {
  mockSelectQueue.push(
    ...
    () => mockChainSelect([{ id: 'driver-1', fleet_id: 'fleet-1', completed_rides_count: 0, rating: '4.5' }]),
...
function queueLegacySelects() {
  mockSelectQueue.push(
    () => mockChainSelect([{ id: 'user-1' }]),
    () => mockChainSelect([{ id: 'driver-1', fleet_id: 'fleet-1', completed_rides_count: 10, rating: '4.8' }]),
    () => mockChainSelect([]),
    () => mockChainSelect([]),
  );
}
