...
Prerequisite: Test Data Seed (Run Once)
Before any test:

Ensure the Supabase local/dev environment has a test driver account (+8801712345678) that is pre‑approved (drivers.status = 'active').

Ensure the admin has created a Micro‑trial package (call_count: 5, is_trial: true) and at least one Standard package (e.g., call_count: 50).

Ensure pricing rows exist for all 8 vehicle types.

For OTP tests: In the dev environment, configure Supabase to accept a known static OTP (123456) for the test phone number (or instruct the Maestro flow to read the SMS using adb logcat).
...
Enter +8801712345678 (or a test number).
...
Wait for SMS auto‑read (or inject 123456 via Maestro’s inputText).
...
Use the Rider app (or direct POST /api/ride/request) to create a ride with vehicle_type = 'car_comfort' and pickup within the active zone.

Receive & Accept Offer:

Wait for ride:offer WebSocket event.

Verify UI: Offer Sheet (Screen 7) appears with:

15‑second countdown ring.

Pickup/dropoff addresses.

Fare (driver_fare_bdt).

Swipe to Accept (fetch:confirm sent).

Verify call deduction: subscriptions.calls_remaining decrements from 5 to 4.

Verify rides.status = 'matched' (auto‑transitions to driver_arriving).

Navigate to Pickup:

Verify the full‑screen map shows route polyline.

Tap I'VE ARRIVED AT PICKUP (Screen 7b).

Verify rides.status = 'driver_arrived', arrived_at set.

Wait for 60‑second timer → verify it auto‑transitions to in_progress (or tap Start Ride early).

Complete Ride:

Verify rides.status = 'in_progress'.

Tap Complete Ride.

Verify rides.status = 'completed', completed_at set.

Verify rider_payable_bdt > 0 (fare recalculated).

Verify Driver Home reflects remaining calls (now 3).
...
Wait for ride:matched WebSocket event.

Verify Rider sees:

Driver name, vehicle number, rating.

Live driver location (blue dot moving on map).

When Driver taps I've Arrived, verify the rider app shows "Your driver has arrived" banner.

When Driver taps Start Ride, verify status = 'in_progress' and the map shows the moving route.

Completion & Rating:

Wait for Driver to tap Complete Ride.

Verify the Final Fare Summary screen (Screen 7c) appears with:

Base, distance, time, promo (if any), total payable.

Tap OK – Got it.

Verify the Rating screen loads.

Select 5 stars.

Tap Submit.

Verify the Thanks confirmation screen appears.

Tap OK → return to Rider Home.
