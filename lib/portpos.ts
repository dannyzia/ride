...
const BASE_URL =
  process.env.PORTPOS_BASE_URL ?? "https://api-sandbox.portpos.com";
...
function generateAuthToken(): string {
  if (!isConfigured()) {
    throw new Error(
      "PortPos not configured (PORTPOS_APP_KEY, PORTPOS_SECRET_KEY required)",
    );
  }
...
export const portposClient = {
  ...
  async createInvoice(
    ...
    logger.info("[PortPos] createInvoice", {
      reference: params.reference,
      status: res.status,
    });
