...
import { accountingAccounts, accountingEntries, accountingEntryLines, riderSubscriptions } from '@/src/db/schema';
...
export async function recordRideCompletion(ride: {
  ...
  if (vat.taxRateId) {
    ledgerPromises.push(recordTaxLedger({ taxRateId: vat.taxRateId, referenceType: 'ride_commission', referenceId: ride.id,
  ...
  if (sourceTax.taxRateId) {
    ledgerPromises.push(recordTaxLedger({ taxRateId: sourceTax.taxRateId, referenceType: 'driver_payout', referenceId: ride.id,
