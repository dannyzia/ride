...
async function seedViaApi() {
  if (!SUPABASE_PAT) {
    console.error('SUPABASE_PAT env var required for API fallback. Get it from: https://supabase.com/dashboard/account/tokens');
    process.exit(1);
  }
...
async function seed() {
  ...
  try {
    const sql = postgres(process.env.DATABASE_URL, { connection: { attempts: 1 } });
