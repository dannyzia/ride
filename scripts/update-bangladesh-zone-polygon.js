// scripts/update-bangladesh-zone-polygon.js
// Updates the active zones.polygon with a simplified Bangladesh mainland border.
// Source: https://github.com/ifahimreza/bangladesh-geojson (src/data/bangladesh.geojson)
// This script uses a pre-extracted ~100 point simplified polygon.
// For a production-quality polygon, fetch the GeoJSON, extract the main polygon,
// simplify with @turf/simplify (tolerance=0.01), and replace the array below.
// Usage: node scripts/update-bangladesh-zone-polygon.js
require('dotenv/config');
const { Client } = require('pg');

// Simplified Bangladesh mainland border (~100 points)
// Extracted from bangladesh-geojson admin polygons, exterior ring only.
// Coordinates converted from [lng, lat] to {lat, lng}.
...
async function update() {
  ...
  try {
    const res = await client.query(
      ...
      [JSON.stringify(BANGLADESH_POLYGON)],
    );

    if (res.rowCount === 0) {
      console.warn('No active zone found. Inserting a default zone.');
      await client.query(
        `INSERT INTO zones (name, polygon, is_active, created_at, updated_at)
         VALUES ('Bangladesh Operational Zone', $1::jsonb, true, now(), now())`,
        [JSON.stringify(BANGLADESH_POLYGON)],
      );
      console.log('Default zone inserted with Bangladesh polygon.');
    } else {
      console.log(`Updated active zone polygon with ${BANGLADESH_POLYGON.length} points.`);
    }

    const verify = await client.query(
      `SELECT id, name, jsonb_array_length(polygon) as point_count, is_active FROM zones WHERE is_active = true`,
    );
